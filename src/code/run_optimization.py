...
def main(args):
    ...
    torch.save(latent_code_init, f"results/original.pt")
    ...
    for i in pbar:
        ...
        if args.save_intermediate_image_every > 0 and i % args.save_intermediate_image_every == 0:
            with torch.no_grad():
                img_gen, latent_save = g_ema([latent], input_is_latent=True, randomize_noise=False, return_latents=True, truncation=0.7, truncation_latent=mean_latent)
            torchvision.utils.save_image(img_gen, f"results/{str(i).zfill(5)}.png") # normalize=True, range=(-1, 1)
            torch.save(latent_save, f"results/{str(i).zfill(5)}.pt")
    ...
    if args.mode == "edit":
        ...
        torch.save(latent_save, f"results/final.pt")
    else:
